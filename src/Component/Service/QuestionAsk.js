        ...
        <div>
            <div className='lg:w-11/12 w-full mx-auto flex flex-wrap lg:my-10 my-5 py-10 lg:h-[400px] h-auto items-center'>
                <div className='lg:text-left text-center lg:w-6/12 w-full'>
                    <h1 className='text-xl font-bold text-orange-400 font-Montserrat'>- ServiceFaqs</h1>
                    <h1 className='lg:text-5xl text-2xl font-bold font-Montserrat text-custom-navy-blue'>Your Questions, Our Responses</h1>
                </div>
                <div className="w-full lg:w-6/12 mx-auto  my-5 md:mb-0">
                    {faqData.map((item, index) => (
                        <div key={index} className="faq-item">
                            <div className="h-auto text-xl lg:font-bold font-semibold  bg-custom-voilet my-1 text-white flex justify-between items-center px-4 mx-4 cursor-pointer" onClick={() => toggleContent(index)}>
                                <h1 className='font-Montserrat lg:text-lg font-semibold text-base p-1'>{item.question}</h1>
                            ...
                            <div className={`faq-answer ${item.isOpen ? 'open' : ''}`}>
                                <p className="p-6 text-lg font-poppins">{item.answer}</p>
                            </div>
                        </div>
                    ))}
                </div>
